...
RSpec.describe 'ツイート投稿', type: :system do
  ...
  context 'ツイート投稿ができるとき'do
    it 'ログインしたユーザーは新規投稿できる' do
      ...
      expect(page).to have_selector ".content_post[style='background_image: url(#{@tweet_image});']"
...
Rspec.describe 'ツイート編集', type: :system do
  ...
  context 'ツイート編集ができるとき' do
    it 'ログインしたユーザーは自分が投稿したツイートの編集ができる' do
      ...
      fill_in 'tweet_text',with: "#{@tweet1.text}+編集したテキスt"
      #編集してもTweetモデルカウントは変わらないことを確認する
      expect{
        find('input[name="commit"]').click
...
RSpec.describe 'ツイート詳細', type: :system do
  ...
  it 'ログインしていない状態でツイート詳細ページに遷移できるもののコメント投稿欄が表示されない' do
    ...
    expect(page).to have_content 'コメントの投稿には新規投稿/ログインが必要です'
  end
end